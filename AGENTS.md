...
- **Don't hand wallet creation back to the web app.** barkd reads a caller-supplied mnemonic as a recovery and rejects it on a bitcoind chain source without a birthday height, which the app cannot supply — `GET /api/v1/bitcoin/tip` needs an open wallet. The `create-wallet` oneshot posts with no mnemonic so barkd generates the seed and starts from the tip.
- **When repinning the chain source in `config.toml`, delete `esplora_address` — don't just add the bitcoind keys beside it.** barkd prefers esplora when both are set. And never write the file before a wallet exists: barkd rejects creation with `Cannot provide an existing config file and config flags`.
