...
- **Don't drop the `log-cap` daemon or the `.bark/debug.log` backup exclude.** barkd's file logger is pinned to trace in `bark-cli/src/log.rs`, and neither `--quiet` nor `BARK_LOG` reaches it — they shape only the terminal logger — so nothing upstream bounds that file.
