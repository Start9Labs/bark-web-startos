...
- **`--expose-mnemonic` is load-bearing — don't drop it from barkd's command.** Upstream made the mnemonic endpoint opt-in (404 by default), and the Settings screen is the only place a user can read the phrase after onboarding: a wallet created before bark-web 0.8.0 was never shown one, the create flow lets them skip the confirmation step, and an imported wallet never shows it. Verified against the 0.6.0 binary: 404 without it, live with it.
...
- **Don't drop the `log-cap` daemon or the `.bark/debug.log` backup exclude.** bark 0.6.2 gave the file logger the terminal logger's level and added `--no-logfile`, but nothing upstream rotates or bounds the file, and the wallet's Settings screen downloads it — so cap it, don't disable it.
