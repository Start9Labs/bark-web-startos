...
3. Start the service, then open the **Web UI** interface from the service's **Dashboard** tab and log in with those credentials.
...
Because of this, this package keeps a **continuous backup**: every change to your wallet is sent to a target you choose, instead of relying only on periodic StartOS backups.
...
**Set it up:** Open **Actions → Continuous Backups → Configure Continuous Backups**, fill in one or more external targets — **Google Drive, Dropbox, Nextcloud, SFTP** (e.g. a NAS or another always-on machine) — and switch on their **Enabled** toggle. Your entries are saved even if you don't enable a target yet, and toggling one off later keeps its credentials, so you never have to re-type them. For **Google Drive or Dropbox**, submit once with your app credentials to get a sign-in link, approve it, then paste the code it shows you back into the form. For **Nextcloud**, enter the address you open it at (its WebDAV address works too) and an app password; for a Nextcloud on your LAN with a self-signed certificate, turn on **Trust self-signed certificate**. For **SFTP**, use a password or an SSH private key without a passphrase; the folder path is relative to the directory your SFTP login starts in — your home directory on most servers, somewhere else on a NAS or a chrooted account; connect with an SFTP client and run `pwd` to see it. After saving, run **Back Up Now** to confirm it works. From then on, every change to your wallet is snapshotted and uploaded within seconds, into a folder named for this wallet inside the folder you chose, so several wallets can share one account.
